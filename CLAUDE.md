...
- **Biome** — Linting and formatting for frontend/config only (src/, *.json, *.ts at root). Rust is linted by `cargo clippy`. `biome.json` explicitly ignores `src-tauri/`, `dist/`, `target/`, `.claude/`.
...
Providers: `none`, `ollama`, `claude`, `gemini`, `openai`, `deepseek`, `cli` — all through one typed Rust router with a shared `reqwest::Client`. Profile context injected server-side into all prompts. `ai_runs` table logs every call.
