...
**Goal:** Build a multi-provider AI router in Rust that routes typed requests to Claude, OpenAI, Gemini, Deepseek, or Ollama, logs every call to `ai_runs`, and injects user profile context.

**Architecture:** A single `ai_request` async function resolves model policy → provider + model, builds the system prompt with profile injection, makes the HTTP call to the provider's API, logs the result to `ai_runs`, and returns the response. Each provider has its own `call_*` function handling API-specific formatting.

**Tech Stack:** Rust (reqwest async, serde_json, Tauri 2), existing AppConfig/AiConfig/ProfileConfig
...
|------|--------|----------------|
| `src-tauri/src/services/ai.rs` | Create | AI router: types, provider calls, model resolution, logging, tests |
| `src-tauri/src/services/mod.rs` | Modify | Export ai module |
| `src-tauri/src/commands/ai.rs` | Create | check_status IPC command |
| `src-tauri/src/commands/mod.rs` | Modify | Export ai module |
| `src-tauri/src/lib.rs` | Modify | Register ai command |

---

### Task 1: AI Service — Types, Model Resolution, Profile Injection

**Files:**
- Create: `src-tauri/src/services/ai.rs`
- Modify: `src-tauri/src/services/mod.rs`

- [ ] **Step 1: Create ai.rs with types, model resolution, and profile injection**
...
pub fn resolve_model(config: &AiConfig, policy: &str) -> (String, String) {
    ...
    let override_model = match provider {
    ...
    if !override_model.is_empty() {
        return (provider.to_string(), override_model.clone());
    }
...
fn sanitize_error(err: &str, config: &AiConfig) -> String {
    ...
    for key in keys {
        if !key.is_empty() && sanitized.contains(key) {
...
Expected: All 12 new AI tests + existing tests pass.

- [ ] **Step 4: Commit**

```bash
git add src-tauri/src/services/ai.rs src-tauri/src/services/mod.rs
git commit -m "feat: add multi-provider AI router with model resolution, profile injection, and logging"
```

---

### Task 2: AI Command — IPC
...
git commit -m "feat: add check_ai_status IPC command"
```
...
- [ ] **Step 2: Verify ai_runs logging works**

In `cargo test`, the `log_ai_run` function writes to `ai_runs`. Verify by adding a quick test or checking existing test DB has the table ready.

- [ ] **Step 3: Summary**

The AI router is complete:
- 5 providers (Claude, OpenAI, Gemini, Deepseek, Ollama)
- Model policy resolution (cheap_local, balanced, strong_reasoning)
- Profile injection into system prompts
- Error sanitization (no API key leakage)
- `ai_runs` logging for every call
- `check_ai_status` IPC for Settings page
- 12 unit tests covering model resolution, profile injection, sanitization, status checks
